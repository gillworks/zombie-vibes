...
export function createZombie(scene, x, y, z, gameState) {
  ...
  zombie.update = function(playerPosition) {
    ...
    if (distanceToPlayer < zombie.userData.attackRadius) {
      ...
      if (now - zombie.userData.lastAttackTime > zombie.userData.attackCooldown) {
    ...
    switch (zombie.userData.type) {
      ...
      case 'Spitter':
        ...
        if (distanceToPlayer < zombie.userData.attackRadius && 
          ...
          if (now - zombie.userData.lastAttackTime > zombie.userData.attackCooldown * 2) {
            zombie.userData.lastAttackTime = now;
            
            // "Spit" at player (visual effect)
            const spitGeometry = new THREE.SphereGeometry(0.2);
            const spitMaterial = new THREE.MeshBasicMaterial({ color: 0x00FF00 });
            ...
            const animateSpit = function() {
              ...
              if (spit.position.distanceTo(zombie.position) > zombie.userData.attackRadius) {
                scene.remove(spit);
                return;
              }
